...
```
Donc df (DataFrame) est une table de données (comme un tableau Excel)
```
...
###### df.head()
```
Affiche les 5 premières lignes du df
```
###### df.tail()
```
Affiche les 5 dernières lignes du df
```
###### df.columns
```
Donne la liste des noms de colonnes
```
###### df.dtypes
```
Donne le type de données de chaque colonne
```
###### df.info
```
Donne un résumé global du df
```
###### df.describe()
```
Donne des statistiques sur les colonnes numériques
```
###### df.dropna()
```
...
![Histogramme](figures/histogram_log2C.png)

##### 5. Quelles remarques peut-on faire à l'observation de l'histogramme et de la loi théorique?

```
Les données sont globalement proches d’une loi normale, mais avec quelques valeurs extrêmes.
```
